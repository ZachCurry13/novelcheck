...
| System page | Admins get a **System** tab with **Check everything**, which tests the AI provider (and fallback), Open Library, Google Books, Calibre library and Content server, email login, remote access, Ollama and the update check, each with a suggested fix. |
