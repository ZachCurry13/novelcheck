...
* **KOReader OPDS catalog:** `opds_tokens` (user, 40-hex token). `GET /opds/{token}` (outside `/api`, no session: the token is the key) returns an OPDS 1.2 acquisition feed (Atom) of the person's Up Next (queued and reading) books that have a file in Calibre, each with an `http://opds-spec.org/acquisition` link to `GET /opds/{token}/book/{id}/{name}`; both apply the person's content rules and answer 404 when the token is unknown or `module_koreader` is off. Links use the address the device used (`X-Forwarded-Proto` honoured when `NOVELCHECK_TRUST_PROXY`). API: `GET /api/me/opds`, `POST /api/me/opds/reset`, `GET /api/admin/users/{id}/opds` (editors: kid accounts only). `web/static/js/delivery.js` shows the address, a QR code (vendored `qrcode-generator`, MIT, `web/static/vendor/qrcode.esm.js`) and KOReader steps, and the Send-to-Kindle confirmation with the approved-sender steps; `/api/me` includes `delivery_from` (the `smtp_from` setting).
* **Barcode scanning & wishlist:** `web/static/js/barcode.js` scans EAN-13 ISBNs (978/979) live: `BarcodeDetector` when the browser has it, otherwise the vendored ZXing UMD build (`web/static/vendor/zxing.min.js`, Apache-2.0, loaded on demand); needs a secure context and `Permissions-Policy: camera=(self)`; camera errors (blocked, missing, busy) fall back to the photo buttons. `Store.Owned` (a copy outside `Looked up`) decides the actions: owned → Add to Up Next; not owned → wishlist or "Up Next (to get)"; delete requests are refused for unowned books. `wishlist` (book, user, note, status `wanted|approved|acquired|declined`, one open wish per book and user); listing marks wishes acquired once the book is owned. Queue items carry `owned` ("📦 Pending acquisition", no Start Reading). API: `POST|DELETE /api/books/{id}/wish` and `GET /api/wishlist` (any user; parents see everyone's), `POST /api/wishlist/{id}/approve|decline|acquired` (editors and admins); `GET /api/books/{id}` and `POST /api/check` include `my_wish` (and `owned`).
...
34. **v1.17 barcode and wishlist.** `web/static/js/barcode.js` + vendored ZXing, `Permissions-Policy` camera for self, `wishlist` table and `internal/store/wishlist.go`, `internal/api/wishlist_handlers.go`, `web/static/js/wishlist.js` (`#/wishlist`), owned-aware actions in Check a book, the book window and Up Next.
35. **v1.17 delivery.** `opds_tokens` table and `internal/store/opds.go`, `internal/api/opds.go` (feed and downloads), `internal/api/koreader_handlers.go`, `web/static/js/delivery.js` (KOReader setup with QR, Send-to-Kindle confirmation), vendored `qrcode-generator`, admin guide step.
