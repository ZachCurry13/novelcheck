...
import { $, esc, attempt, classChip, flagChips } from "./ui.js";
...
export async function openBook(id, state, onChange) {
  ...
  dlg.innerHTML = `
    <div class="max-h-[85vh] overflow-y-auto p-6 space-y-4">
      ...
      ${b.status === "error" && isAdmin ? `<p class="text-sm text-rose-400">Last error: ${esc(b.analysis_error)}</p>` : ""}
      ...
      ${b.analysis_model ? `<p class="text-xs text-slate-500">Analyzed by ${esc(b.analysis_model)}</p>` : ""}
      <div class="flex flex-wrap gap-2 pt-2">
        ...
        ${isAdmin ? `<button data-act="analyze" class="btn-secondary">${b.classification ? "Re-analyze" : "Analyze now"}</button>` : ""}
      </div>
    </div>`;
  dlg.onclick = async (e) => {
